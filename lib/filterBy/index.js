...
module.exports = function filter(env, query, options) {
  'use strict';
  ...
  if (latestFilter && packageName) {
    return filterByPackage(env, packageName, query, options)
      .then((ids) => {
        return filterByLatest(env, { scope: ids });
      });
  }
  else if (packageName) {
    return filterByPackage(env, packageName, query, options);
  ...
  if (latestFilter) {
    return filterByLatest(env, options);
  }
