...
function applyProjection(env, results, options, projection, returnFields) {
  return Promise.map(results, (item) => {
...
DB.prototype.find = function doFind(query, cursor) {
  ...
  return findByDataset.find(query, cursor)
    .then((result) => {
      ...
      return filterBy(query, options);
    })
    .then((results) => {
      options.scope = results;
      if (query.listing && options.scope.length > 0) {
    ...
    .then((results) => {
      options.scope = results;
      return findByIndex.find(query.manifest, 'manifest', options);
    })
    .then((results) => {
    ...
    .then((results) => {
      ...
      if (projection.listing) {
        return returnWithListing(query.listing, options)
          .then((listingResult) => {
    ...
    .then((results) => {
      let finalOptions = filterIds(results, options, 'uiResult');
      finalOptions = filterIds(results, finalOptions, 'manifestResult');
      finalOptions = filterIds(results, finalOptions, 'listingResult');
