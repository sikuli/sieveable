...
apps.init = function (type) {
    return new Promise(function (resolve, reject) {
        if (type === 'ui') {
            client.smembers(uiKey, function (err, res) {
                if (err) {
                    reject(err);
                }
                apps.ids = res;
            ...
            client.smembers(manifestKey, function (err, res) {
                if (err) {
                    reject(err);
                }
                apps.ids = res;
            ...
            client.smembers(codeKey, function (err, res) {
                if (err) {
                    reject(err);
                }
                apps.ids = res;
            ...
            reject(new Error("Unknown findBy dataset type"));
        }
    });
};
