 ...
 */
apps.project = function (id, options, projection, returnFields) {
...
function _renameReturnFields(resultObj, returnFields, prefix) {
    ...
    var fields = _.filter(returnFields, function (f) {
        return f.trim().toLowerCase().indexOf(prefix) == 0;
    });
    if (resultObj && resultObj.returnAttributes) {
        if (fields.length > 0) {
            var keys = Object.keys(resultObj.returnAttributes);
            _.forEach(fields, function (f, idx) {
                var asIndex = f.trim().toLowerCase().indexOf('as');
                var newName;
                if (asIndex > -1) {
                    newName = f.trim().substring(asIndex + 2, f.length);
                }
                if (newName.length > 0) {
                    var newPropertyName = newName.replace(/ /g, '');
