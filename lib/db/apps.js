...
apps.project = function applyProjection(id, options, projection, returnFields) {
  const app = projection.app ? getApp(id) : {},
    listing = projection.listing ? getProjectionAttributes(options.listingResult,
      id) : {},
    ui = projection.ui ? getProjectionAttributes(options.uiResult, id) : {},
    manifest = projection.manifest ? getProjectionAttributes(options.manifestResult,
      id) : {};
  renameReturnFields(listing, returnFields, 'l$');
  renameReturnFields(manifest, returnFields, 'm$');
  renameReturnFields(ui, returnFields, 'u$');
  return getPathById(app.id)
    .then((appPath) => {
      ...
      return {
        ...
        listing: listing.returnAttributes || {},
        ui: ui.returnAttributes || {},
        manifest: manifest.returnAttributes || {}
      };
    });
};
...
function renameReturnFields(result, returnFields, prefix) {
...
function getProjectionAttributes(results, id) {
