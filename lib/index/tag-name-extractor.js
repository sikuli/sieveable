  ...
  return Promise.map(xmlFileNames, (xmlFile) => {
    ...
    return extract(xmlFile, readStream, writeStream)
        ...
        .catch(FileExtractingError, (e) => {
          log.error('FileExtractingError in %s. Reason: %s ', e.fileName, e.message);
          fs.unlink(outFile);
        })
        .catch((e) => {
          log.error(e);
...
function extract(fileName, readStream, writeStream) {
  return new Promise((resolve, reject) => {
    const saxStream = sax.createStream(strict, {
      ...
      .on('error', (e) => {
        reject(new FileExtractingError('SAX Parser Error in %s. Reason: %s',
          e.message, fileName));
      })
      .on('opentag', (node) => {
        readStream.pause();
    ...
    writeStream.on('error', (error) => {
      reject(new FileExtractingError('Failed to write output file %s. Reason: %s',
        fileName, error.message));
    });

    readStream.on('error', (error) => {
      log.error('An error has occurred while reading from file %s. Reason: ',
        fileName, error.message);
      reject(error);
    });
