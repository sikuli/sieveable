  ...
  return new Promise((resolve, reject) => {
    ...
    _.forEach(fileElements, (fileElement) => {
      ...
      if (rootElementList.length !== 1) {
        ...
        reject(new FileExtractingError(
    ...
    writeStream.on('finish', () => {
      resolve('Suffix paths have been extracted for %s', xmlFile);
    });
    writeStream.end();
  });
}
...
function writeAncestors(element, writeStream) {
  ...
  while (newElem.parentNode && newElem.parentNode.localName !== 'File') {
    newElem = newElem.parentNode;
    if (newElem.nodeType === 1) {
      ancestors.push(newElem.localName);
    }
  }
  let line = '';
