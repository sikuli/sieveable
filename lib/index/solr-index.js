...
exports.commit = function commit(collectionName) {
  const solrUpdateUrl =
    `http://${config.get('dbConfig.solr.host')}:${config.get('dbConfig.solr.port')}/solr/${collectionName}/update`;
  ...
  return request.postAsync({ headers: { 'content-type': 'application/json' },
    ...
    .then((body) => {
      ...
      throw new Error('Failed to commit index changes for collection %s. % Reason: %s',
        collectionName, JSON.stringify(resHeader));
    })
    .catch((e) => {
...
exports.indexFile = function indexFile(fileNames, extention, collectionName) {
  const solrExtractUrl =
    `http://${config.get('dbConfig.solr.host')}:${config.get('dbConfig.solr.port')}/solr/${collectionName}/update/extract`;
    ...
    return request.postAsync({ url: solrExtractUrl, formData: solrUpdateParams })
        ...
        .then((body) => {
          ...
          if (resHeader.status === 0) {
            ...
            throw new FileIndexingError('File indexing failed for %s. Response: %s',
              fileName, JSON.stringify(body));
          }
        })
