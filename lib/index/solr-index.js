...
exports.indexFile = function indexFile(fileNames, extention, collectionName, callback) {
...
exports.indexListing = function indexListing(content) {
    try {
        const solrUpdateUrl = 'http://' + config.get('dbConfig.solr.host') + ':' +
            config.get('dbConfig.solr.port') +
            '/solr/' + config.get('dbConfig.solr.listingCollection') + '/update',
            ...
            .then((body) => {
                if (body.responseHeader.status === 0) {
                    ...
                    throw new Error('Listing detail indexing failed for ' + id +
                        'Response Header: ' + JSON.stringify(body.responseHeader));
                }
            })
            .catch((e) => {
