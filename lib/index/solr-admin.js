  ...
  return request.getAsync({ url: solrUrl, qs: { action: 'LIST', wt: 'json' } })
    .spread((response, body) => {
      if (response.statusCode !== 200) {
        throw new Error('Failed to find collection: ' + collectionName +
          '. Reason: ' + body.error.msg);
      }
      const resObj = JSON.parse(body);
      if (resObj.responseHeader.status !== 0) {
        throw new SolrCollectionError(
          'Solr LIST action has failed. Reason ' +
          body.error.msg);
      }
...
exports.createCollection = function createCollection(collectionName) {
  return exec('solr create -c ' + collectionName + ' -p ' + config.get(
      'dbConfig.solr.port'), { shell: config.get('system.shell') })
    .spread((stdout, stderr) => {
      if (stderr) {
        throw new SolrCollectionError(
          'Failed to create a new collection: ' + collectionName +
          '. Reason: ' + stderr);
      }
...
exports.addField = function addField(collection, field) {
  ...
  return request.postAsync({ url: solrUrl, json: { 'add-field': field } })
    .spread((response, body) => {
      if (response.statusCode === 200 && body.responseHeader.status === 0) {
        if (body.errors === undefined) {
          console.log('Field: ' + field.name +
            ' has been created in collection: ' + collection);
      ...
      throw new SolrCollectionError('Failed to create a new field (' +
        field.name + ') in collection: ' + collection + '. Reason: ' +
