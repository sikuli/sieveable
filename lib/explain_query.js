...
const _ = require('lodash'),
    ...
    log = require('./logger'),
    LISTING_RESERVED = ['listing-details', 'description', 'store-url',
    'store-category', 'price', 'date-published', 'version-name', 'version-code',
    ...
    LISTING_FIELDS = { 'store-category': 'cat', 'content-rating': 'crat',
    'developer': 'crt', 'developer-url': 'curl', 'downloads': 'dct',
    'description': 'desc', 'date-published': 'dtp', 'downloads-count-text': 'dtxt',
    'package-name': 'n', 'whats-new': 'new', 'android-version': 'os', 'price': 'pri',
    'rating': 'rate', 'rating-count': 'rct', 'reviews': 'rev', 'install-size': 'sz',
    'install-size-text': 'sztxt', 'title': 't', 'store-url': 'url',
    'version-code': 'verc', 'version-name': 'vern',
    'listing-details': 'listing-details'
  },
    MANIFEST_RESERVED = ['manifest', 'uses-permission', 'permission', 'permission-tree',
...
function getQueryParts(query) {
    ...
    if (returnIndex > -1) {
        if (limitIndex > -1) {
            ...
            if (modeIndex > -1) {
                queryParts.return = query.substring(returnIndex + 6, modeIndex).trim();
            }
            else {
