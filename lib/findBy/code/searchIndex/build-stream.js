    ...
    _.forEach(files, function (textFile) {
        ...
        request.postAsync({
            ...
            if (resHeader.status == 0) {
                console.log("Successfully updated %s-%s", package_name, version_code)
            }
            else {
                throw Error('Status ' + resHeader.status);
            }
        }).catch(clientError, function (e) {
            console.error("Failed to update %s-%s %s", package_name,
...
function commit() {
    ...
    request.postAsync({
        ...
        var resHeader = JSON.parse(body).responseHeader;
        if (resHeader.status == 0) {
            console.log("Successfully committed all pending operations.")
        }
        else {
            throw Error('Status ' + resHeader.status);
        }
    }).catch(clientError, function (e) {
        console.error("Failed to commit pending updates " + e.message);
    });
}
module.exports = {
