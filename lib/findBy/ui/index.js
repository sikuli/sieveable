...
function _find_by_a_single_tag_with_n_children_of_multiple_tags(q, app) {
    ...
    var childrenOccurrences = new Array()
    q.children.forEach(function (item) {
        childrenNames.push(_get_child_name(item));
        childrenOccurrences.push(item.count)
    });
    var result = _.some($(selector).toArray(), function (elem) {
        var found = false;
        for (var i = 0; i < childrenNames.length; i++) {
            var elemName = _get_element_name(elem)
            var count = $(elemName).children(childrenNames[i]).length
            if (childrenNames[i] == '*') {
                if (count == childrenOccurrences.length) {
                    found = true
                }
                else {
                    found = false;
                ...
                if (count == childrenOccurrences[i]) {
                    found = true;
                }
                else {
                    found = false;
