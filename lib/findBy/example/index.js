...
function _getAttributeSelectors(attributes) {
    ...
    _.forEach(attributes, function (attrib) {
        ...
        if (idx == -1) {
            var attribText= '[' + attrib.name + '="' + attrib.value + '"]';
            attributesSelector += _escapeSpecialCharacters(attribText)
        }
        // Attribute value starts with a value
        else if (idx == attrib['value'].length - 1) {
            var attribText = '[' + attrib.name + '^="' + attrib.value.split('*')[0] + '"]';
            attributesSelector += _escapeSpecialCharacters(attribText)
        }
        // Attribute value ends with a value
        else if (idx == 0) {
            var attribText = '[' + attrib.name + '$="' + attrib.value.split('*')[1] + '"]';
            attributesSelector += _escapeSpecialCharacters(attribText)
        }
    })
