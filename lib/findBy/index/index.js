...
function find(env, query, queryType, options) {
    ...
    if (queryType === "ui") {
        _extractTags(query);
        if (h1Pairs.length > 0 && tagNames.length > 0) {
            return Promise.all([getSolrRequest('uiTagCollection', h1Pairs),
                getSolrRequest('uiTagH1Collection', tagNames)])
                .then(function (ids) {
                    var inter = _.intersection(ids[0], options.scope);
                    return Promise.resolve(_.intersection(ids[1], inter));
                }).catch(function () {
                    return Promise.reject(new Error("Failed to get index results from the uiTagCollection and uiTagH1Collection"));
                });
        }
        else if (h1Pairs.length > 0 && tagNames.length == 0) {
            return getSolrRequest('uiTagH1Collection', h1Pairs)
                .then(function (ids) {
                    return Promise.resolve(_.intersection(ids, options.scope));
                })
                .catch(function () {
                    return Promise.reject(new Error("Failed to get index results from the uiTagH1Collection"));
                });
        }
        else if (tagNames.length > 0 && h1Pairs.length == 0) {
...
function _extractTags(uiQueryText) {
    var doc = new xmldom.DOMParser().parseFromString(uiQueryText,
        "text/xml");
...
function addChildren(element, parent) {
    ...
    if (element.hasChildNodes()) {
        h1Pairs.push(parentName + "-" + elementName);
...
function addTagName(element) {
    //TODO: Add attributes with * and escape solr keywords
    var elementName = element.localName;
    // Skip anonymous tags
    ...
    /*_.forEach(element.attributes, function(attr){
     tagNames.push(element.localName + "(" + attr.nodeName + "=" +
     attr.nodeValue + ")");
     });
     */
