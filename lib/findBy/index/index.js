...
exports.find = function find(env, query, queryType, options) {
  ...
  if (queryType === 'ui') {
    ...
    if (suffixArray.length > 0 && tagNames.length > 0) {
      return Promise.all([getSolrRequest('uiTagCollection', suffixArray, options.scope),
                getSolrRequest('uiSuffixCollection', tagNames, options.scope)])
        .then((ids) => {
          return Promise.resolve(_.intersection(ids[0], ids[1]));
        })
        .catch(() => {
          return Promise.reject(new Error('Failed to get index ' +
            'results from the uiTagCollection and uiSuffixCollection'));
        });
    }
    else if (suffixArray.length > 0 && tagNames.length === 0) {
      return getSolrRequest('uiSuffixCollection', suffixArray, options.scope)
        .then((ids) => {
          return Promise.resolve(ids);
        })
        .catch(() => {
          return Promise.reject(new Error('Failed to get index ' +
            'results from the uiSuffixCollection'));
        });
    }
    else if (tagNames.length > 0 && suffixArray.length === 0) {
      return getSolrRequest('uiTagCollection', tagNames, options.scope)
        .then((ids) => {
          return Promise.resolve(ids);
        })
        .catch(() => {
          return Promise.reject(new Error(
            'Failed to get index results from the uiTagCollection'));
        });
    }
    return Promise.reject(new Error(
        'IndexSearchError: No tags are found for the given query'));
  }
    ...
    return getSolrRequest('manifestCollection', tagNames, options.scope)
      .then((ids) => {
        return Promise.resolve(ids);
      })
      .catch(() => {
        return Promise.reject(new Error(
          'Failed to get index results from the manifestCollection'));
      });
  }
  return Promise.reject(new Error('Unknown index type'));
};

function getSolrRequest(collectionName, arrValues, ids) {
...
function addChildren(element, parent) {
  let parentName = parent.localName,
