...
module.exports = function lookupMatcher(q, app, $) {
  ...
  if (q.children === undefined && q.count === 1) {
    return findBySingleTag(q, $);
  }
  else if (q.children === undefined && q.count > 1) {
    return findBySiblingsOfOneType(q, $);
  }
  else if (q.children === undefined && q.length > 1) {
    return findBySiblingsOfMultipleTypes(q, $);
  }
  else if (q.children && q.children.length === 1) {
    return findByASingleTagWithNChildrenOfOneTag(q, app, $);
  }
  else if (q.children && q.children.length > 1) {
    return findByASingleTagWithNChildrenOfMultipleTags(q, $);
  }
  log.error('Unrecognized UI query.', q);
  return false;
};

function findBySingleTag(q, $) {
  'use strict';
  let found = false,
...
function findByASingleTagWithNChildrenOfOneTag(q, app, dom) {
  'use strict';
  const $ = dom || app,
    selector = matchHelper.getElementName(q),
  ...
  if ('children' in q.children[0] && result.length >= childCount) {
    return findByASingleTagWithNChildrenOfOneTag(q.children[0], result);
  }
  return {
...
function findByASingleTagWithNChildrenOfMultipleTags(q, $) {
...
function findBySiblingsOfOneType(q, $) {
  const selector = matchHelper.getElementName(q),
...
function findBySiblingsOfMultipleTypes(q, $) {
