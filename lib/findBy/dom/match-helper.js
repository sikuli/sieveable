...
function getElementName(element) {
  ...
  if ('attributes' in element) {
    const actualAttributes = filterExtraAttributes(element.attributes);
...
function getChildName(child) {
  ...
  if ('attributes' in child) {
    const actualAttributes = filterExtraAttributes(child.attributes);
...
function getAttributeSelectors(attributes) {
  ...
  _.forEach(attributes, (attribute) => {
    ...
    if (idx === -1) {
      attributesSelector += `[${attribute.name}="${attribute.value}"]`;
    }
    // Attribute value starts with a value
    else if (idx === attribute.value.length - 1) {
      attributesSelector += `[${attribute.name}^="${attribute.value.split('*')[0]}"]`;
    }
    // Attribute value ends with a value
    else if (idx === 0) {
      attributesSelector += `[${attribute.name}$="${attribute.value.split('*')[1]}"]`;
    }
  });
  return attributesSelector;
}

function filterExtraAttributes(attributes) {
